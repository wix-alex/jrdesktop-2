...
public class ScreenPlayer extends JLabel {
    ...
    public void paint(Graphics g) {
     //   super.paint(g);
        g.drawImage(img, 0, 0, screenRectScale.width, screenRectScale.height, this);            
        DrawSelectingRect(g);
    }

    public void UpdateScreen(byte[] data) { 
        screenRect = new Rectangle(recorder.viewerData.getScreenRect());  
        screenRectScale = screenRect;
        
        float screenScale = recorder.viewerData.getScreenScale();   
        screenRectScale.width = (int) (screenRectScale.width * screenScale);
        screenRectScale.height = (int) (screenRectScale.height * screenScale);

        setSize(screenRectScale.getSize());
        setLocation(screenRectScale.getLocation());  
    ...
    {        
        if (isSelecting) {
            ...
            if (PartialScreenMode) {
                ...
                recorder.viewer.updateOptions();                
                recorder.viewerGUI.jBtnPartialComplete.setText("Default screen");
            }
